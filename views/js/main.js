...
var resizePizzas = function (size) {
  ...
  function changePizzaSizes(size) {
    var range = pizzas.length;
    /*var values = [];
    for (var i = 0; i < range; i++) {
      var dx = determineDx(pizzas[i], size);
      values.push((pizzas[i].offsetWidth + dx) + 'px');
    }*/
    var value = (pizzas[0].offsetWidth + determineDx(pizzas[0], size)) + 'px';
    for (var i = 0; i < range; i++) {
      pizzas[i].style.width = value;
    }
  }
...
function updatePositions() {
  ...
  var phase = [];
  phase.push(Math.sin(top + (0 % 5)));
  phase.push(Math.sin(top + (1 % 5)));
  phase.push(Math.sin(top + (2 % 5)));
  phase.push(Math.sin(top + (3 % 5)));
  phase.push(Math.sin(top + (4 % 5)));
  for (var i = 0; i < range; i++) {
    /*phase = Math.sin(top + (i % 5));*/
    values.push(items[i].basicLeft + 100 * phase[i % 5] + 'px');
  }
  for (var i = 0; i < range; i++) {
    items[i].style.left = values[i];
  }
...
document.addEventListener('DOMContentLoaded', function () {
  //no of rows constrained to pizza
  var rows = parseInt(window.screen.height / 356) + 1;
  var cols = 8;
  var s = 256;
  var elem;
  ...
  //items stored globally for use animation
